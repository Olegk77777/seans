...
/* Чип «рулетка по автору» — встаёт на место category-drawer */
.author-chip {
  display: flex;
  align-items: center;
  gap: 14px;
...
.author-chip .clear:hover { color: var(--alert); }
.author-chip .clear svg { width: 14px; height: 14px; }
.filters {
  display: flex;
  flex-wrap: wrap;
  gap: 6px;
...
.poet-head .poet-roulette:hover { color: var(--accent); border-color: var(--accent); }
...
function declExcerpts(n) {
  const m10 = n % 10, m100 = n % 100;
  if (m10 === 1 && m100 !== 11) return 'отрывок';
  if (m10 >= 2 && m10 <= 4 && (m100 < 12 || m100 > 14)) return 'отрывка';
  return 'отрывков';
}

function Roulette({ data, openPoem, favs, toggleFav, onAuthorClick, lockedCategory, authorFilter, clearAuthorFilter }) {
  ...
  const deckBase = useMemo(() => {
    // Режим «рулетка по автору»: только его стихи с текстом + его цитаты,
    // категории игнорируются (drawer в этом режиме скрыт).
    if (authorFilter) {
      return [
        ...POEMS.filter(p => p.hasText && p.author === authorFilter),
        ...QUOTES.filter(qt => qt.author === authorFilter),
      ];
    }
  ...
  }, [category, POEMS, QUOTES, authorFilter]);
  ...
  useEffect(() => { setIdx(0); }, [category, authorFilter]);
  ...
  if (!shuffled.length) {
    return e(Fragment, null,
      e('div', { className:'hero' },
        ...
        e('p', { className:'lede' }, authorFilter
          ? ('У автора «' + authorFilter + '» пока нет текстов для рулетки.')
          : category === 'fav'
          ? 'В избранном пока ничего нет — нажмите ♥ у любого отрывка.'
          : 'База пуста — нажмите «Обновить», чтобы подтянуть тексты.')),
      e('div', { className:'actions' },
        authorFilter
          ? e('button', { className:'btn', onClick: clearAuthorFilter }, '← Ко всем отрывкам')
          : category === 'fav'
  ...
  return e(Fragment, null,
    e('div', { className:'hero' },
      e('div', { className:'kicker' }, isFav ? 'Ваша коллекция' : authorFilter ? 'Рулетка по автору' : 'Литературная рулетка'),
      authorFilter
        ? e('h1', null, e('em', null, authorFilter))
      ...
      e('p', { className:'lede' }, isFav
        ...
        : authorFilter
          ? (deckBase.length + ' ' + declExcerpts(deckBase.length) + ' — ' + Math.min(CARDS_PER_PAGE, deckBase.length) + ' в каждый приход.')
    ...
    e('div', { className:'toolbar' },
      e('div', { className:'search-box' }, e(Icon.search),
        e('input', { placeholder: isFav ? 'Поиск среди избранного…' : authorFilter ? 'Поиск среди отрывков автора…' : 'Поиск среди отрывков…', value: query, onChange: ev => setQuery(ev.target.value) }),
        e('span', { className:'shortcut' }, '⌘K')),
      authorFilter && e('div', { className:'author-chip' },
        'Автор',
        e('span', { className:'name' }, authorFilter),
        e('button', { className:'clear', onClick: clearAuthorFilter, title:'Сбросить фильтр — ко всем отрывкам' }, e(Icon.x))),
      !lockedCategory && !authorFilter && e('details', { className:'filter-drawer', ref: filterRef },
        e('summary', null,
...
function Library({ data, openPoem, favs, toggleFav, prefillAuthor, clearPrefill, onAuthorRoulette }) {
  const { POETS } = data;
        ...
        return e('div', { key: poet.name, 'data-poet': poet.name, className: 'poet-block ' + (isOpen?'open':'') },
          e('div', { className:'poet-head', onClick: () => !q && setOpenPoet(isOpen ? null : poet.name) },
            ...
            e('div', null,
              ...
              poet.poems.some(pp => pp.hasText) ? e('button', {
                className:'poet-roulette', title:'Крутить рулетку только по этому автору',
                onClick: ev => { ev.stopPropagation(); onAuthorRoulette(poet.name); } },
                '✦ рулетка') : null),
...
function App() {
  ...
  // Фильтр «рулетка по одному автору» — включается кнопкой ✦ в библиотеке.
  const [rouletteAuthor, setRouletteAuthor] = useState(null);

  const [backStack, setBackStack] = useState([]);
  const currentRef = useRef({ section, litSub, cineSub, rouletteAuthor });
  useEffect(() => { currentRef.current = { section, litSub, cineSub, rouletteAuthor }; }, [section, litSub, cineSub, rouletteAuthor]);
  ...
  const navLitSub = (v) => {
    if (v === litSub) {
      // Повторный клик по «Рулетке» в режиме автора — сброс на общую рулетку.
      if (v === 'roulette' && rouletteAuthor) { pushBack(); setRouletteAuthor(null); }
      return;
    }
    pushBack();
    setLitSub(v);
    // Клик по подтабу «Рулетка» всегда открывает общую рулетку без фильтра.
    if (v === 'roulette') setRouletteAuthor(null);
  };
  const navCineSub = (v) => {
    if (v === cineSub) return;
    pushBack();
  ...
  const goBack = () => {
    ...
    setRouletteAuthor(last.rouletteAuthor || null);
  ...
  // Из библиотеки — в рулетку только по одному автору (кнопка ✦ у поэта).
  const goToRouletteByAuthor = (author) => {
    pushBack();
    setSection('lit');
    setLitSub('roulette');
    setRouletteAuthor(author);
  };
  ...
  const litBody = litSub === 'library'
    ? e(Library, { data, openPoem: setOpenedPoem, favs, toggleFav,
        ...
        onAuthorRoulette: goToRouletteByAuthor })
    : litSub === 'favorites'
      ...
      : e(Roulette, { data, openPoem: setOpenedPoem, favs, toggleFav, onAuthorClick: goToAuthor,
          authorFilter: rouletteAuthor,
          clearAuthorFilter: () => { pushBack(); setRouletteAuthor(null); } });
