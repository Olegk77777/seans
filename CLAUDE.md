...
### Автокоммит и пуш — включено (правило для Claude)

После любой правки `app.py` / `docs/` / UI выполняй полный workflow Pages
...
- Если push идёт в feature-ветку (worktree, например `claude/...`) —
  Pages **не обновится** (Pages смотрит только на `main`/`/docs`).
  В этом случае коммит и пуш в ветку делать молча, но **обязательно
  предупредить пользователя**, что для деплоя нужен merge в `main`.
- Перед коммитом — короткий `git status` + `git diff --stat`, чтобы
  убедиться, что не подбираю мусор.

**Когда НЕ автокоммитить (спросить):**
