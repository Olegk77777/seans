...
### ⚠️ Олег смотрит ОСНОВНОЙ бандл — доводи правки до `main`

Олег запускает `Сеанс.app` **только из основной папки**
`~/Codex/AskBukaPoetry/` (ветка `main`). Лончер при старте читает
`Contents/Resources/app.py` именно оттуда. Если Claude работает в worktree
(`.claude/worktrees/...`, ветка `claude/...`), правки `app.py` **не видны
в приложении Олега**, пока не попали в `main` основного дерева — ни
перезапуск, ни кэш ни при чём. (Реальная путаница 29 мая 2026: фикс
строфики лежал в worktree-ветке, а приложение брало старый код из `main`.)

Поэтому, закончив и закоммитив правку в worktree-ветке:

1. Влей её в `main` основного дерева **локальным fast-forward** (БЕЗ
   удалённого пуша — пуш остаётся за Олегом):
   ```
   git -C ~/Codex/AskBukaPoetry merge --ff-only <имя-ветки>
   ```
   Это обновит и `app.py`, и копию `Resources/app.py` в основном бандле.
2. **Напомни Олегу**: чтобы увидеть изменения — закрыть и заново открыть
   `Сеанс.app` (код и кэш стихов читаются один раз при старте).
3. Когда всё проверено и работает — **напомни про `git push` на GitHub**
   (Олег пушит сам; для веб-зеркала Pages нужен ещё
   `build_static.py --snapshot` + push в `main`).

### Автокоммит — да, автопуш — нет (правило для Claude)

Олег учится работать с git и хочет сам инициировать каждый пуш. Поэтому
после правки `app.py` / `docs/` / UI выполняй workflow **только до коммита**:

1. `python3 build_static.py` (или `--snapshot`, если правились данные).
2. `cp app.py "Сеанс.app/Contents/Resources/app.py"` (если правил `app.py` в корне).
3. `git add` ровно тех файлов, что менялись (никаких `git add -A`).
4. `git commit -m "..."` со стилем сообщения как в `git log` —
   lowercase префикс + русское описание (`ui: ...`, `fix(pages): ...`,
   `docs: ...`, `gallery: ...`). Без `Co-Authored-By` строк.
5. **СТОП. Не пушить.** В конце ответа явно напомнить Олегу одной строкой:

   > 📌 Закоммитил, но **не запушил**. Скажи «запушь» — отправлю.
   > Сейчас ветка: `<имя ветки>`, для деплоя Pages нужен push в `main`.

   Если ветка — `main` или fast-forward в main очевиден, добавь подсказку
   про команду, которую Олег может выполнить сам:
   ```
   git push origin HEAD:main
   ```

   Когда Олег ответит «запушь» / «пуш» / «отправь» — тогда `git push`
   (или `git push origin HEAD:main` для fast-forward из worktree).

**Что соблюдать всегда:**
- Не пропускать pre-commit хуки (`--no-verify` запрещён).
- Никаких `--force`, `reset --hard`, амендов опубликованных коммитов.
- Worktree-ветки вида `claude/...` пушатся в одноимённую remote-ветку
  через `git push origin HEAD`. Pages при этом **не обновится** —
  Pages смотрит только на `main`/`/docs`. Для деплоя нужен либо
  fast-forward `git push origin HEAD:main`, либо merge через PR.
- Перед коммитом — короткий `git status` + `git diff --stat`, чтобы
  убедиться, что не подбираю мусор.

**Когда НЕ автокоммитить (спросить перед коммитом):**
- Если правил больше, чем просили (рефакторинг, удаление файлов).
- Если в diff попали файлы, которых пользователь не касался
  (`favorites.json`, `screenshots.json` без явной просьбы и т.п.).
- Если что-то упало в `build_static.py` или тестах.
